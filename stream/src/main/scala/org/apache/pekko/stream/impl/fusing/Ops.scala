...
private[pekko] final class Gather[In, Out](factory: () => Gatherer[In, Out]) extends GraphStage[FlowShape[In, Out]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with InHandler with OutHandler {
      ...
      // After a successful push the pull is inlined, removing the maybePull/maybeRunFinalAction
      // call chain that was adding 2–3 extra virtual dispatches per element.
